                        ...
                        <tbody>
                            @forelse ($cart_items as $item)
                                <tr wire:key="{{ $item['product_id'] }}">
                                    ...
                                    <td class="py-4">
                                        <div class="flex items-center">
                                            <button wire:click="decreaseQtyCartItems({{ $item['product_id'] }})" class="border rounded-md py-2 px-4 mr-2">-</button>
                                            <span class="text-center w-8">{{ $item['quantity'] }}</span>
                                            <button wire:click="increaseQtyCartItems({{ $item['product_id'] }})" class="border rounded-md py-2 px-4 ml-2">+</button>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <button wire:click="removeItem({{ $item['product_id'] }})" class="bg-slate-300 border-2 border-slate-400 rounded-lg px-3 py-1 hover:bg-red-500 hover:text-white hover:border-red-700">
                                            ...
                                            <span wire:loading wire:target="removeItem({{ $item['product_id'] }})">Wait...</span>
                                        </button>
                                    </td>
                                </tr>    
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center p-4 text-lg font-semibold text-slate-400">No items available in cart!</td>
                                </tr>
                            @endforelse
